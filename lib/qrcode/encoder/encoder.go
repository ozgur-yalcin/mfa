...
func Encoder_encodeWithoutHint(content string, ecLevel decoder.ErrorCorrectionLevel) (*QRCode, lib.WriterException) {
	return Encoder_encode(content, ecLevel, nil)
}

func Encoder_encode(content string, ecLevel decoder.ErrorCorrectionLevel, hints map[lib.EncodeHintType]interface{}) (*QRCode, lib.WriterException) {
	encoding := Encoder_DEFAULT_BYTE_MODE_ENCODING
	encodingHint, hasEncodingHint := hints[lib.EncodeHintType_CHARACTER_SET]
	if hasEncodingHint {
		if eci, ok := common.GetCharacterSetECIByName(fmt.Sprintf("%v", encodingHint)); ok {
			encoding = eci.GetCharset()
		} else {
			return nil, lib.NewWriterException(encodingHint)
		}
	}

	mode := chooseMode(content, encoding)

	headerBits := lib.NewEmptyBitArray()
	...
	gs1FormatHint, hasGS1FormatHint := hints[lib.EncodeHintType_GS1_FORMAT]
	if hasGS1FormatHint {
	...
	if versionHint, ok := hints[lib.EncodeHintType_QR_VERSION]; ok {
		...
		if !willFit(bitsNeeded, version, ecLevel) {
			return nil, lib.NewWriterException("Data too big for requested version")
		}
	} else {
	...
	headerAndDataBits := lib.NewEmptyBitArray()
	...
	maskPattern := -1
	if hintMaskPattern, ok := hints[lib.EncodeHintType_QR_MASK_PATTERN]; ok {
...
func recommendVersion(ecLevel decoder.ErrorCorrectionLevel, mode *decoder.Mode,
	headerBits *lib.BitArray, dataBits *lib.BitArray) (*decoder.Version, lib.WriterException) {
...
func calculateBitsNeeded(
	mode *decoder.Mode,
	headerBits *lib.BitArray,
	dataBits *lib.BitArray,
...
func chooseMaskPattern(bits *lib.BitArray, ecLevel decoder.ErrorCorrectionLevel,
	version *decoder.Version, matrix *ByteMatrix) (int, lib.WriterException) {
	...
	for maskPattern := 0; maskPattern < QRCode_NUM_MASK_PATERNS; maskPattern++ {
		...
		if e != nil {
			return -1, lib.WrapWriterException(e)
		}
...
func chooseVersion(numInputBits int, ecLevel decoder.ErrorCorrectionLevel) (*decoder.Version, lib.WriterException) {
	...
	return nil, lib.NewWriterException("Data too big")
}
...
func terminateBits(numDataBytes int, bits *lib.BitArray) lib.WriterException {
	capacity := numDataBytes * 8
	if bits.GetSize() > capacity {
		return lib.NewWriterException(
	...
	if bits.GetSize() != capacity {
		return lib.NewWriterException("bits.GetSize()=%d, capacity=&d", bits.GetSize(), capacity)
	}
	return nil
}

func getNumDataBytesAndNumECBytesForBlockID(numTotalBytes, numDataBytes, numRSBlocks, blockID int) (int, int, lib.WriterException) {
	if blockID >= numRSBlocks {
		return 0, 0, lib.NewWriterException("Block ID too large")
	}
	...
	if numEcBytesInGroup1 != numEcBytesInGroup2 {
		return 0, 0, lib.NewWriterException("EC bytes mismatch")
	}
	if numRSBlocks != numRsBlocksInGroup1+numRsBlocksInGroup2 {
		return 0, 0, lib.NewWriterException("RS blocks mismatch")
	}
	if numTotalBytes !=
		((numDataBytesInGroup1+numEcBytesInGroup1)*numRsBlocksInGroup1)+
			((numDataBytesInGroup2+numEcBytesInGroup2)*numRsBlocksInGroup2) {
		return 0, 0, lib.NewWriterException("Total bytes mismatch")
	}
...
func interleaveWithECBytes(bits *lib.BitArray, numTotalBytes, numDataBytes, numRSBlocks int) (*lib.BitArray, lib.WriterException) {

	if bits.GetSizeInBytes() != numDataBytes {
		return nil, lib.NewWriterException("Number of bits and data bytes does not match")
	}
	...
	if numDataBytes != dataBytesOffset {
		return nil, lib.NewWriterException("Data bytes does not match offset")
	}

	result := lib.NewEmptyBitArray()
...
func generateECBytes(dataBytes []byte, numEcBytesInBlock int) ([]byte, lib.WriterException) {
...
func appendModeInfo(mode *decoder.Mode, bits *lib.BitArray) {
	_ = bits.AppendBits(mode.GetBits(), 4)
}

func appendLengthInfo(numLetters int, version *decoder.Version, mode *decoder.Mode, bits *lib.BitArray) lib.WriterException {
...
func appendBytes(content string, mode *decoder.Mode, bits *lib.BitArray, encoding textencoding.Encoding) lib.WriterException {
	switch mode {
	...
	default:
		return lib.NewWriterException("Invalid mode: %v", mode)
	}
}

func appendNumericBytes(content string, bits *lib.BitArray) {
	length := len(content)
	i := 0
	for i < length {
...
func appendAlphanumericBytes(content string, bits *lib.BitArray) lib.WriterException {
	length := len(content)
	i := 0
	for i < length {
		code1 := getAlphanumericCode(content[i])
		if code1 == -1 {
			return lib.NewWriterException("appendAlphanumericBytes")
		}
		if i+1 < length {
...
func append8BitBytes(content string, bits *lib.BitArray, encoding textencoding.Encoding) lib.WriterException {
	bytes := []byte(content)

	var e error
	bytes, e = encoding.NewEncoder().Bytes([]byte(content))
	if e != nil {
		return lib.WrapWriterException(e)
	}
...
func appendKanjiBytes(content string, bits *lib.BitArray) lib.WriterException {
	enc := common.StringUtils_SHIFT_JIS_CHARSET.NewEncoder()
	bytes, e := enc.Bytes([]byte(content))
	if e != nil {
		return lib.WrapWriterException(e)
	...
	if len(bytes)%2 != 0 {
		return lib.NewWriterException("Kanji byte size not even")
	}
	maxI := len(bytes) - 1
	for i := 0; i < maxI; i += 2 {
		...
		if subtracted == -1 {
			return lib.NewWriterException("Invalid byte sequence")
		}
...
func appendECI(eci *common.CharacterSetECI, bits *lib.BitArray) {
