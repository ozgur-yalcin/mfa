...
func embedBasicPatterns(version *decoder.Version, matrix *ByteMatrix) lib.WriterException {
...
func embedTypeInfo(ecLevel decoder.ErrorCorrectionLevel, maskPattern int, matrix *ByteMatrix) lib.WriterException {
	typeInfoBits := lib.NewEmptyBitArray()
...
func maybeEmbedVersionInfo(version *decoder.Version, matrix *ByteMatrix) lib.WriterException {
	if version.GetVersionNumber() < 7 {
		return nil
	}
	versionInfoBits := lib.NewEmptyBitArray()
...
func embedDataBits(dataBits *lib.BitArray, maskPattern int, matrix *ByteMatrix) lib.WriterException {
	bitIndex := 0
	direction := -1
	x := matrix.GetWidth() - 1
...
func makeTypeInfoBits(ecLevel decoder.ErrorCorrectionLevel, maskPattern int, bits *lib.BitArray) lib.WriterException {
	if !QRCode_IsValidMaskPattern(maskPattern) {
		return lib.NewWriterException("Invalid mask pattern")
	}
...
func makeVersionInfoBits(version *decoder.Version, bits *lib.BitArray) lib.WriterException {
...
func embedDarkDotAtLeftBottomCorner(matrix *ByteMatrix) lib.WriterException {
	if matrix.Get(8, matrix.GetHeight()-8) == 0 {
		return lib.NewWriterException("embedDarkDotAtLeftBottomCorner")
	}
	matrix.Set(8, matrix.GetHeight()-8, 1)
	return nil
}

func embedHorizontalSeparationPattern(xStart, yStart int, matrix *ByteMatrix) lib.WriterException {
	for x := 0; x < 8; x++ {
...
func embedVerticalSeparationPattern(xStart, yStart int, matrix *ByteMatrix) lib.WriterException {
	for y := 0; y < 7; y++ {
...
func embedPositionDetectionPatternsAndSeparators(matrix *ByteMatrix) lib.WriterException {
