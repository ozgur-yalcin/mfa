...
func DecodedBitStreamParser_Decode(
	...
	hints map[lib.DecodeHintType]interface{}) (*common.DecoderResult, error) {
	...
	for {
		...
		default:
			...
			default:
				return nil, lib.NewFormatException("Unknown mode")
			}
			break
		}
...
func DecodedBitStreamParser_decodeHanziSegment(bits *common.BitSource, result []byte, count int) ([]byte, error) {
	if count*13 > bits.Available() {
		return result, lib.NewFormatException("bits.Available() = %v", bits.Available())
	}
	...
	if e != nil {
		return result, lib.WrapFormatException(e)
	}
	return result, nil
}
...
func DecodedBitStreamParser_decodeByteSegment(bits *common.BitSource,
	...
	byteSegments [][]byte, hints map[lib.DecodeHintType]interface{}) ([]byte, [][]byte, error) {

	if 8*count > bits.Available() {
		return result, byteSegments, lib.NewFormatException("bits.Available = %v", bits.Available())
	}
	...
	if currentCharacterSetECI == nil {
		...
		if err != nil {
			return nil, nil, lib.WrapFormatException(err)
		}
	} else {
	...
	if e != nil {
		return result, byteSegments, lib.WrapFormatException(e)
	}
...
func toAlphaNumericChar(value int) (byte, error) {
	if value >= len(ALPHANUMERIC_CHARS) {
		return 0, lib.NewFormatException("%v >= len(ALPHANUMERIC_CHARS)", value)
	}
...
func DecodedBitStreamParser_decodeNumericSegment(bits *common.BitSource, result []byte, count int) ([]byte, error) {
	for count >= 3 {
		...
		if threeDigitsBits >= 1000 {
			return result, lib.NewFormatException("threeDigitalBits = %v", threeDigitsBits)
		}
	...
	if count == 2 {
		...
		if twoDigitsBits >= 100 {
			return result, lib.NewFormatException("twoDigitsBits = %v", twoDigitsBits)
		}
		...
		if digitBits >= 10 {
			return result, lib.NewFormatException("digitBits = %v", digitBits)
		}
...
func DecodedBitStreamParser_parseECIValue(bits *common.BitSource) (int, error) {
	...
	if e != nil {
		return -1, lib.WrapFormatException(e)
	}
	if (firstByte & 0x80) == 0 {
	...
	return -1, lib.NewFormatException()
}
