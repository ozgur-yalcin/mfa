...
func NewFinderPatternFinder(image *lib.BitMatrix, resultPointCallback lib.ResultPointCallback) *FinderPatternFinder {
...
func (f *FinderPatternFinder) GetImage() *lib.BitMatrix {
	return f.image
}
...
func (f *FinderPatternFinder) Find(hints map[lib.DecodeHintType]interface{}) (*FinderPatternInfo, lib.NotFoundException) {
	...
	bl, tl, tr := lib.ResultPoint_OrderBestPatterns(fps[0], fps[1], fps[2])
...
func (f *FinderPatternFinder) SelectBestPatterns() ([]*FinderPattern, lib.NotFoundException) {
	startSize := float64(len(f.possibleCenters))
	if startSize < 3 {
		return nil, lib.NewNotFoundException("startSize = %v", startSize)
	}
	...
	if distortion == math.MaxFloat64 {
		return nil, lib.NewNotFoundException("module size is too different")
	}
