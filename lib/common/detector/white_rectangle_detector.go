...
func NewWhiteRectangleDetectorFromImage(image *lib.BitMatrix) (*WhiteRectangleDetector, error) {
	return NewWhiteRectangleDetector(
		image, whiteRectangleDetector_INIT_SIZE, image.GetWidth()/2, image.GetHeight()/2)
}

func NewWhiteRectangleDetector(image *lib.BitMatrix, initSize, x, y int) (*WhiteRectangleDetector, error) {
	halfsize := initSize / 2
...
func (this *WhiteRectangleDetector) Detect() ([]lib.ResultPoint, error) {
	left := this.leftInit
	...
	if !sizeExceeded {
		...
		var z lib.ResultPoint
		for i := 1; z == nil && i < maxSize; i++ {
			z = this.getBlackPointOnSegment(left, down-i, left+i, down)
		}

		if z == nil {
			return nil, lib.NewNotFoundException("no black point on left-down")
		}
		...
		if t == nil {
			return nil, lib.NewNotFoundException("no black point on left-up")
		}
		...
		if x == nil {
			return nil, lib.NewNotFoundException("no black point on right-up")
		}
		...
		if y == nil {
			return nil, lib.NewNotFoundException("no black point on right-down")
		}
...
func (this *WhiteRectangleDetector) getBlackPointOnSegment(aX, aY, bX, bY int) lib.ResultPoint {
	...
	for i := 0; i < dist; i++ {
		...
		if this.image.Get(x, y) {
			return lib.NewResultPoint(float64(x), float64(y))
		}
	}
	return nil
}

func (this *WhiteRectangleDetector) centerEdges(y, z, x, t lib.ResultPoint) []lib.ResultPoint {
	yi := y.GetX()
	yj := y.GetY()
	zi := z.GetX()
	zj := z.GetY()
	...
	if yi < float64(this.width)/2.0 {
		return []lib.ResultPoint{
			lib.NewResultPoint(ti-whiteRectangleDetector_CORR, tj+whiteRectangleDetector_CORR),
			lib.NewResultPoint(zi+whiteRectangleDetector_CORR, zj+whiteRectangleDetector_CORR),
			lib.NewResultPoint(xi-whiteRectangleDetector_CORR, xj-whiteRectangleDetector_CORR),
			lib.NewResultPoint(yi+whiteRectangleDetector_CORR, yj-whiteRectangleDetector_CORR),
		}
	} else {
		return []lib.ResultPoint{
			lib.NewResultPoint(ti+whiteRectangleDetector_CORR, tj+whiteRectangleDetector_CORR),
