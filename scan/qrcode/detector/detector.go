...
func NewDetector(image *scan.BitMatrix) *Detector {
	return &Detector{image, nil}
}

func (this *Detector) GetImage() *scan.BitMatrix {
	return this.image
}

func (this *Detector) GetResultPointCallback() scan.ResultPointCallback {
...
func (this *Detector) Detect(hints map[scan.DecodeHintType]interface{}) (*common.DetectorResult, error) {
	if hints != nil {
		if cb, ok := hints[scan.DecodeHintType_NEED_RESULT_POINT_CALLBACK]; ok {
			this.resultPointCallback, _ = cb.(scan.ResultPointCallback)
		}
	}
...
func (this *Detector) ProcessFinderPatternInfo(info *FinderPatternInfo) (*common.DetectorResult, error) {
	...
	if moduleSize < 1.0 {
		return nil, scan.NewNotFoundException("moduleSize = %v", moduleSize)
	}
	...
	if len(provisionalVersion.GetAlignmentPatternCenters()) > 0 {
		...
		for i := 4; i <= 16; i <<= 1 {
			...
			if e == nil {
				break
			} else if _, ok := e.(scan.NotFoundException); !ok {
				return nil, e
			}
		}
	...
	if alignmentPattern == nil {
		points = []scan.ResultPoint{bottomLeft, topLeft, topRight}
	} else {
		points = []scan.ResultPoint{bottomLeft, topLeft, topRight, alignmentPattern}
	}
	return common.NewDetectorResult(bits, points), nil
}

func Detector_createTransform(topLeft, topRight, bottomLeft scan.ResultPoint, alignmentPattern *AlignmentPattern, dimension int) *common.PerspectiveTransform {
...
func Detector_sampleGrid(image *scan.BitMatrix, transform *common.PerspectiveTransform, dimension int) (*scan.BitMatrix, error) {
	sampler := common.GridSampler_GetInstance()
	return sampler.SampleGridWithTransform(image, dimension, dimension, transform)
}

func (this *Detector) computeDimension(topLeft, topRight, bottomLeft scan.ResultPoint, moduleSize float64) (int, error) {
	tltrCentersDimension := util.MathUtils_Round(scan.ResultPoint_Distance(topLeft, topRight) / moduleSize)
	...
	case 2:
		...
		break
	case 3:
		return 0, scan.NewNotFoundException("dimension = %v", dimension)
	}
...
func (this *Detector) calculateModuleSizeOneWay(pattern, otherPattern scan.ResultPoint) float64 {
...
func (this *Detector) findAlignmentInRegion(overallEstModuleSize float64, estAlignmentX, estAlignmentY int, allowanceFactor float64) (*AlignmentPattern, error) {
	...
	if x := float64(alignmentAreaRightX - alignmentAreaLeftX); x < overallEstModuleSize*3 {
		return nil, scan.NewNotFoundException("x = %v, moduleSize = %v", x, overallEstModuleSize)
	}
	...
	if y := float64(alignmentAreaBottomY - alignmentAreaTopY); y < overallEstModuleSize*3 {
		return nil, scan.NewNotFoundException("y = %v, moduleSize = %v", y, overallEstModuleSize)
	}
