...
import (
	...
	"github.com/ozgur-yalcin/mfa/scan/qrcode/encoder"
)
...
func (this *QRCodeWriter) EncodeWithoutHint(
	contents string, format scan.BarcodeFormat, width, height int) (*scan.BitMatrix, error) {
	return this.Encode(contents, format, width, height, nil)
}

func (this *QRCodeWriter) Encode(
	contents string, format scan.BarcodeFormat, width, height int,
	hints map[scan.EncodeHintType]interface{}) (*scan.BitMatrix, error) {

	if len(contents) == 0 {
		return nil, scan.NewWriterException("IllegalArgumentException: Found empty contents")
	}

	if format != scan.BarcodeFormat_QR_CODE {
	...
	if hints != nil {
		if ec, ok := hints[scan.EncodeHintType_ERROR_CORRECTION]; ok {
			if ecl, ok := ec.(decoder.ErrorCorrectionLevel); ok {
				...
				if e != nil {
					return nil, scan.NewWriterException("EncodeHintType_ERROR_CORRECTION: %w", e)
				}
				errorCorrectionLevel = ecl
			} else {
				return nil, scan.NewWriterException(
		...
		if m, ok := hints[scan.EncodeHintType_MARGIN]; ok {
			if qz, ok := m.(int); ok {
				quietZone = qz
			} else if str, ok := m.(string); ok {
				qz, e := strconv.Atoi(str)
				if e != nil {
					return nil, scan.NewWriterException("EncodeHintType_MARGIN = \"%v\": %w", m, e)
				}
				quietZone = qz
			} else {
				return nil, scan.NewWriterException(
...
func renderResult(code *encoder.QRCode, width, height, quietZone int) (*scan.BitMatrix, error) {
	...
	if input == nil {
		return nil, scan.NewWriterException("IllegalStateException")
	}
	...
	output, e := scan.NewBitMatrix(outputWidth, outputHeight)
	if e != nil {
