...
type BitMatrixParser struct {
	bitMatrix        *scan.BitMatrix
	parsedVersion    *Version
	parsedFormatInfo *FormatInformation
	mirror           bool
}

func NewBitMatrixParser(bitMatrix *scan.BitMatrix) (*BitMatrixParser, error) {
	dimension := bitMatrix.GetHeight()
	if dimension < 21 || (dimension&0x03) != 1 {
		return nil, scan.NewFormatException("dimension = %v", dimension)
	}
...
func (this *BitMatrixParser) ReadFormatInformation() (*FormatInformation, error) {
	...
	return nil, scan.NewFormatException("failed to parse format info")
}
