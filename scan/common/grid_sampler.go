...
type GridSampler interface {
	SampleGrid(image *scan.BitMatrix, dimensionX, dimensionY int,
	...
	SampleGridWithTransform(image *scan.BitMatrix,
...
func GridSampler_checkAndNudgePoints(image *scan.BitMatrix, points []float64) error {
	...
	for offset := 0; offset < maxOffset && nudged; offset += 2 {
		...
		if x < -1 || x > width || y < -1 || y > height {
			return scan.NewNotFoundException(
